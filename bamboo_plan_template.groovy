...
./sbt -Dsbt.log.noformat=true ";set credentials:=Seq(Credentials(\\"Sonatype Nexus Repository Manager\\", \\"maven.atlassian.com\\", \\"${USER}\\", \\"${PWD}\\")); set useGpg := true; set pgpPassphrase := Some(Array()); release with-defaults" -J-Xmx2G
''')
